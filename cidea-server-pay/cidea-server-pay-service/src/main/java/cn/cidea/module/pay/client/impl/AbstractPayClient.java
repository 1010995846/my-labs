package cn.cidea.module.pay.client.impl;
...
import cn.cidea.module.pay.client.exception.PayException;
...
import cn.cidea.module.pay.utils.ValidationUtils;
...
public abstract class AbstractPayClient<Config extends PayClientConfig> implements PayClient {
    ...
    public final PayOrderRespDTO unifiedOrder(PayOrderUnifiedReqDTO reqDTO) {
        ...
        } catch (BizException ex) { // 业务异常，都是实现类已经翻译，所以直接抛出即可
            throw ex;
        } catch (Throwable ex) {
            // 系统异常，则包装成 PayException 异常抛出
            log.error("[unifiedOrder][客户端({}) request({}) 发起支付异常]",
                    getId(), JSONObject.toJSONString(reqDTO), ex);
