package cn.cidea.module.pay.client.impl.alipay;
...
public abstract class AbstractAlipayPayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    public PayOrderRespDTO doParseOrderNotify(Map<String, String> params, String body) throws Throwable {
        ...
        PayOrderStatusEnum status = parseStatus(bodyObj.get("trade_status"));
        // 特殊逻辑: 支付宝没有退款成功的状态，所以，如果有退款金额，我们认为是退款成功
        if (MapUtil.getDouble(bodyObj, "refund_fee", 0D) > 0) {
            status = PayOrderStatusEnum.REFUND;
        }
    ...
    protected PayOrderRespDTO doGetOrder(String outTradeNo) throws Throwable {
        ...
        // 2.2 解析订单的状态
        PayOrderStatusEnum status = parseStatus(response.getTradeStatus());
        Assert.notNull(status, (Supplier<Throwable>) () -> {
    ...
    private static PayOrderStatusEnum parseStatus(String tradeStatus) {
        return Objects.equals("WAIT_BUYER_PAY", tradeStatus) ? PayOrderStatusEnum.WAITING
                : StringUtils.equalsAny(tradeStatus, "TRADE_FINISHED", "TRADE_SUCCESS") ? PayOrderStatusEnum.SUCCESS
                : Objects.equals("TRADE_CLOSED", tradeStatus) ? PayOrderStatusEnum.CLOSED : null;
    }
