package cn.cidea.module.pay.client.impl.weixin;

import cn.cidea.module.pay.client.impl.AbstractPayClient;
import cn.cidea.module.pay.dataobject.dto.PayOrderRespDTO;
import cn.cidea.module.pay.dataobject.dto.PayOrderUnifiedReqDTO;
import cn.cidea.module.pay.dataobject.dto.PayRefundRespDTO;
import cn.cidea.module.pay.dataobject.dto.PayRefundUnifiedReqDTO;
import cn.cidea.module.pay.dataobject.enums.PayOrderStatusEnum;
import cn.cidea.module.pay.utils.FileUtils;
...
public abstract class AbstractWxPayClient extends AbstractPayClient<WxPayClientConfig> {
    ...
    protected WxPayUnifiedOrderV3Request buildPayUnifiedOrderRequestV3(PayOrderUnifiedReqDTO reqDTO) {
        ...
        request.setAmount(new WxPayUnifiedOrderV3Request.Amount().setTotal(reqDTO.getAmt())); // 单位分
    ...
    private PayOrderRespDTO doParseOrderNotifyV2(String body) throws WxPayException {
        ...
        PayOrderStatusEnum status = Objects.equals(response.getResultCode(), "SUCCESS") ?
                PayOrderStatusEnum.SUCCESS : PayOrderStatusEnum.CLOSED;
    ...
    private PayOrderRespDTO doParseOrderNotifyV3(String body) throws WxPayException {
        ...
        // 2. 构建结果
        PayOrderStatusEnum status = parseStatus(result.getTradeState());
    ...
    private PayOrderRespDTO doGetOrderV2(String outTradeNo) throws WxPayException {
        ...
        // 转换结果
        PayOrderStatusEnum status = parseStatus(response.getTradeState());
        return PayOrderRespDTO.of(status, response.getTransactionId(), response.getOpenid(), parseDateV2(response.getTimeEnd()),
                outTradeNo, response);
    }
    ...
    private static PayOrderStatusEnum parseStatus(String tradeState) {
        switch (tradeState) {
            case "NOTPAY":
            case "USERPAYING": // 支付中，等待用户输入密码（条码支付独有）
                return PayOrderStatusEnum.WAITING;
            case "SUCCESS":
                return PayOrderStatusEnum.SUCCESS;
            case "REFUND":
                return PayOrderStatusEnum.REFUND;
            case "CLOSED":
