 ...
 */
@Component
public class PmPropFactory {

    @Autowired
    private IPmPropMapper baseMapper;

    public Builder build(Serializable id) {
        PmProp entity = baseMapper.selectById(id);
        Assert.notNull(entity, "id异常");
        return new Builder(Collections.singleton(entity));
    }

    public Builder build(PmProp entity) {
        if (entity == null) {
    ...
    public Builder build(Set<Serializable> ids) {
        ...
        Collection<PmProp> coll = baseMapper.selectBatchIds(ids);
        return new Builder(coll);
    }

    public Builder build(Collection<PmProp> coll) {
        if (coll == null) {
            return new Builder(new ArrayList<>(0));
        }
        return new Builder(coll);
    ...
    public class Builder {
        ...
        private Collection<PmProp> coll;
