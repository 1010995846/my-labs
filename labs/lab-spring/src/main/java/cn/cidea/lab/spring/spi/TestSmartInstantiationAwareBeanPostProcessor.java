...
public class TestSmartInstantiationAwareBeanPostProcessor implements SmartInstantiationAwareBeanPostProcessor {
    ...
    /**
     * 该触发点发生在postProcessBeforeInstantiation之前，这个方法用于预测Bean的类型，返回第一个预测成功的Class类型，如果不能预测返回null；
     * 当你调用BeanFactory.getType(name)时当通过bean的名字无法得到bean类型信息时就调用该回调方法来决定类型信息。
     */
    @Override
    public Class<?> predictBeanType(Class<?> beanClass, String beanName) throws BeansException {
        if (set.add("predictBeanType")) {
            log.info("Custom predictBeanType");
        }
    ...
    public Constructor<?>[] determineCandidateConstructors(Class<?> beanClass, String beanName) throws BeansException {
        if (set.add("determineCandidateConstructors")) {
            log.info("Custom determineCandidateConstructors");
        }
    ...
    public Object getEarlyBeanReference(Object bean, String beanName) throws BeansException {
        if (set.add("getEarlyBeanReference")) {
            log.info("Custom getEarlyBeanReference");
        }
