...
public abstract class Express implements IExpress {
    ...
    static class InHqlExpress extends Express {
        protected Hql right;

        private InHqlExpress(String left, Hql right) {
            super.left = left;
            this.right = right;
        }
        ...
        @Override
        public StringBuilder generateExpress() {
            return new StringBuilder(left).append("IN (").append(right.generate()).append(")");
        }
    }
    ...
    public static Express in(String name, Hql value){
        return new InHqlExpress(name, value);
    }
